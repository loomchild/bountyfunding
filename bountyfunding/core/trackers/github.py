...
def sync_issues(project_id):
    api = GithubApi(token=config[project_id].GITHUB_TOKEN)
    #TODO: retrieve path from project
    github_issues = api.get('/repos/loomchild/sandbox/issues')
    updated_issues = []

    #TODO: add paging
    for github_issue in github_issues:
        issue = create_update_issue_from_github_issue(project_id, github_issue)
        button = update_button(project_id, github_issue)
        if issue or button:
...
def update_button(project_id, github_issue):
    issue_ref = github_issue.number
    body = github_issue.body
    body = remove_button(body)
    body = add_button(body, issue_ref)
    
    if body != github_issue.body:
        api = GithubApi(token=config[project_id].GITHUB_TOKEN)
        #TODO: retrieve path from project
        api.patch('/repos/loomchild/sandbox/issues/%s' % issue_ref, data=dict(body=body))
        return True
...
def add_button(body, issue_ref):
    #TODO: retrieve url from config
    url = "http://bountyfunding.ngrok.io"
    #TODO: move image to different path with project
    image_url = url + "/issue/%s.svg" % issue_ref
    #TODO: retrieve project path from config
    issue_url = url + "/projects/sandbox/issues/%s.html" % issue_ref
    button = "[![Bounty](%s)](%s)\n" % (image_url, issue_url)
    return button + body
